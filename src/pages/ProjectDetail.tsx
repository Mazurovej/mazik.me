...
import { GitBranch, ExternalLink, CheckCircle2, Terminal } from 'lucide-react';
        ...
        <div className="w-full max-w-2xl space-y-4">
          ...
          >
            ...
            <div className="bg-surface border border-border rounded-2xl p-6">
              <h2 className="text-xs font-bold text-zinc-500 uppercase tracking-widest mb-4">Tech Stack</h2>
          ...
          {project.devNotes.length > 0 && (
            <motion.div 
              initial={emilInitial}
              animate={emilEnter}
              transition={{ ...emilTransition, delay: 0.3 }}
              className="bg-surface border border-border rounded-[2rem] p-6 md:p-8"
            >
              <div className="flex items-center gap-2 mb-6">
                <Terminal className="w-5 h-5 text-minecraft-green" />
                <h2 className="text-xs font-bold text-zinc-500 uppercase tracking-widest">Developer Notes</h2>
              </div>
              
              <div className="space-y-4">
                {project.devNotes.map((note, idx) => (
                  <div key={idx} className="bg-[#0f0f11] border border-border/50 rounded-2xl p-6 relative overflow-hidden group">
                    <div className="absolute top-0 left-0 w-1 h-full bg-minecraft-green/20 group-hover:bg-minecraft-green transition-colors" />
                    <h3 className="text-xs font-mono font-bold text-minecraft-green mb-3 tracking-wide flex items-center gap-2">
